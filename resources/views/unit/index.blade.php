...
@if (Sentinel::getUser()->hasAccess(['unit.store']))
...
@include('unit.create')
@endif
@if (Sentinel::getUser()->hasAccess(['unit.update']))
@include('unit.edit')
@endif
...
<div class="kt-portlet kt-portlet--mobile">
    ...
    <div class="kt-portlet__body">
        <!--begin: Datatable -->
        <table class="table table-striped- table-bordered table-hover table-checkable" id="tbl_unit">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Unit Name</th>
...
{!! Form::open(['method'=>'DELETE', 'route' => ['unit.destroy', 0], 'style' => 'display:inline','id'=>'form-delete'])
!!}
{!! Form::close() !!}
@stop
...
<script type="text/javascript">
            ...
            columnDefs: [{
                ...
                render: function (data, type, full, meta) {
                    ...
                    data + `,'` + full.name + `')">@lang('global.app_edit') {{$name}}</a>` +
                        @endif
                    @if(Sentinel::getUser()->hasAccess(['unit.destroy']))
                    ` <a class="btn btn-danger btn-sm" onclick="delete_unit(` + data +
    ...
    function delete_unit(id) {
        swal.fire({
            ...
            if (result.value) {
                ...
                $('#form-delete').attr('action', "{{route('unit.index')}}/" + id);
                $('#form-delete').submit();
    ...
    function edit(id, name) {
        if ($('#edit').is(":visible") && id == id_awal) {
            ...
            $('#edit').show('500');
            $('#e_name').val(name);
            $('#form-update').attr('action', "{{route('unit.index')}}/" + id);
        }
