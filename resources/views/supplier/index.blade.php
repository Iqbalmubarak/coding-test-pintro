...
@if (Sentinel::getUser()->hasAccess(['supplier.store']))
<a href="javascript::void(0)" onclick="$('#create').toggle(500);$('#edit').hide(500);"
...
@endif
@if (Sentinel::getUser()->hasAccess(['supplier.update']))
...
<div class="kt-portlet kt-portlet--mobile">
    ...
    <div class="kt-portlet__body">
        <!--begin: Datatable -->
        <table class="table table-striped- table-bordered table-hover table-checkable" id="tbl_supplier">
            <thead>
                <tr>
                    <th>No</th>
                    <th>{{$name}} Name</th>
...
{!! Form::open(['method'=>'DELETE', 'route' => ['supplier.destroy', 0], 'style' => 'display:inline','id'=>'form-delete'])
!!}
{!! Form::close() !!}
@stop
...
<script type="text/javascript">
        ...
        var table = $('#tbl_supplier').DataTable({
            ...
            columnDefs: [{
                ...
                render: function (data, type, full, meta) {
                    ...
                    return `` +
                        @if(Sentinel::getUser()->hasAccess(['supplier.update']))
                    ` <a class="btn btn-success btn-sm" href="javascript::void(0)" onclick="edit(` +
                    data + `,'` + full.name + `')">@lang('global.app_edit') {{$name}}</a>` +
                        @endif
                    @if(Sentinel::getUser()->hasAccess(['supplier.destroy']))
                    ` <a class="btn btn-danger btn-sm" onclick="delete_supplier(` + data +
                        `)" href="javascript::void(0)">@lang('global.app_delete') {{$name}}</a>` +
                        @endif ``;
                },
            }, ],
        });
    ...
    function delete_supplier(id) {
        swal.fire({
            ...
            if (result.value) {
                ...
                $('#form-delete').attr('action', "{{route('supplier.index')}}/" + id);
                $('#form-delete').submit();
    ...
    function edit(id, name) {
        if ($('#edit').is(":visible") && id == id_awal) {
            ...
            $('#edit').show('500');
            $('#e_name').val(name);
            $('#form-update').attr('action', "{{route('supplier.index')}}/" + id);
        }
