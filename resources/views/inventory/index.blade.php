...
@if (Sentinel::getUser()->hasAccess(['inventory.create']))
<a href="{{route('inventory.create')}}"
    class="btn btn-label-brand btn-bold">@lang('global.app_create') {{$name}}</a>
@endif
@stop
    ...
    <div class="kt-portlet__head kt-portlet__head--lg">
        <div class="kt-portlet__head-label">
            ...
            <h3 class="kt-portlet__head-title">
                @lang('global.app_list') Inventory
            </h3>
        </div>
    ...
    <div class="kt-portlet__body">
        ...
        <form class="kt-form kt-form--fit kt-margin-b-20">
            <div class="row kt-margin-b-20">
                ...
                <div class="col-lg-4 kt-margin-b-10-tablet-and-mobile">
                    <label>Total:</label>
                    <input type="number" class="form-control kt-input" placeholder=""
                        data-col-index="3">
                </div>
        ...
        <!--begin: Datatable -->
        <table class="table table-striped- table-bordered table-hover table-checkable" id="tbl_inventory">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Product</th>
...
{!! Form::open(['method'=>'DELETE', 'route' => ['inventory.destroy', 0], 'style' =>
'display:inline','id'=>'form-delete']) !!}
{!! Form::close() !!}


<!--begin::Modal-->
<div class="modal fade" id="f_modal_edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="col-md-12">
                    <div class="kt-form__group--inline">
                        <div class="kt-form__label">
                            <label class="kt-label m-label--single">Product Name:</label>
                        </div>
                        <div class="kt-form__control">
                            {!! Form::select('product', $product, null, ['class' => 'form-control kt-select2 myselect2',
                            'id'=>'s_product_update']) !!}
                            @error('product')
                ...
                <div class="col-md-12">
                    <div class="kt-form__group--inline">
                        <div class="kt-form__label">
                            <label class="kt-label m-label--single">Unit Name:</label>
                        </div>
                        <div class="kt-form__control">
                            {!! Form::select('unit', $unit, null, ['class' => 'form-control kt-select2 myselect2',
                            'id'=>'s_unit_update']) !!}
                            @error('unit')
                            <div class="form-text text-danger">{{$message}}</div>
                            @enderror
...
<script type="text/javascript">
        ...
        var table = $('#tbl_inventory').DataTable({
                    ...
                    switch (column.index()) {
                        case 1:
                            column.data().unique().sort().each(function (d, j) {
                                $('.kt-input[data-col-index="1"]').append(
                                    '<option value="' + d + '">' + d +
                                    '</option>');
                            });
                            break;
                        case 2:
                            column.data().unique().sort().each(function (d, j) {
                                $('.kt-input[data-col-index="2"]').append(
                                    '<option value="' + d + '">' + d +
                                    '</option>');
                            });
                            break;
                        case 3:
                            column.data().unique().sort().each(function (d, j) {
                                $('.kt-input[data-col-index="3"]').append(
                                    '<option value="' + d + '">' + d +
                                    '</option>');
                            });
                            break;
            ...
            columnDefs: [{
                ...
                render: function (data, type, full, meta) {
                    ...
                    ` <a class="btn btn-success btn-sm" href="javascript::void(0)" onclick="edit_inventory(` +
                    data + `,` + full.product_id + `,` + full.unit_id +
                        `)">@lang('global.app_edit')</a>` +
                        @endif
                    @if(Sentinel::getUser()->hasAccess(['inventory.show']))
                    ` <a class="btn btn-success btn-sm" href="` + base + '/inventory/' +
                        data + `">@lang('global.app_detail')</a>` +
                        @endif
                    @if(Sentinel::getUser()->hasAccess(['inventory.destroy']))
                    ` <a class="btn btn-danger btn-sm" onclick="delete_inventory(` + data +
                        `)" href="javascript::void(0)">@lang('global.app_delete')</a>` +
                        @endif ``;
                },
            }, ],
        });
    ...
    function delete_inventory(id) {
        swal.fire({
            ...
            if (result.value) {
                ...
                $('#form-delete').attr('action', "{{route('inventory.index')}}/" + id);
                $('#form-delete').submit();
    ...
    function edit_inventory(id, product, unit) {
        $("#s_product_update").val(product).trigger('change');
        $("#s_unit_update").val(unit).trigger('change');
        $('#f_modal_edit').modal('show');
        var base = "{{url('/')}}";
        $('#form_edit').attr('action', base + '/inventory/' + id);
    }
